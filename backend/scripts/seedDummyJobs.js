...
function generateDummyJobs(count = 100) {
  const jobs = [];
  const now = new Date();
...
async function seedDummyJobs() {
  try {
    ...
    const dummyJobs = generateDummyJobs(100);

    // Insert jobs into database
    console.log("Inserting jobs into database...");
    const result = await Job.insertMany(dummyJobs);

    console.log(`\n✅ Successfully created ${result.length} dummy jobs!`);
    console.log(`Total jobs in database: ${await Job.countDocuments()}`);
