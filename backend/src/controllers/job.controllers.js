...
const getJobs = asyncHandler(async (req, res) => {
  ...
  // Build base query with expired job filtering
  const baseFilters = [
    {
      $or: [
        { isGeminiGenerated: false }, // Include all non-Gemini jobs
        { 
          isGeminiGenerated: true, 
          deadline: { $gt: new Date() } // Include only non-expired Gemini jobs
        }
      ]
    },
  ...
  // Job type filter - handle multiple types with $in
  if (req.query.type) {
  ...
  // Work mode filter - handle multiple modes with $in
  if (req.query.workMode) {
    const workModes = Array.isArray(req.query.workMode) ? req.query.workMode : [req.query.workMode];
    baseFilters.push({
  ...
  if (req.query.salaryFrom && req.query.salaryTo) {
    baseFilters.push({
      $or: [
        {
          "salaryRange.from": {
            $gte: parseInt(req.query.salaryFrom),
            $lte: parseInt(req.query.salaryTo),
          },
        },
        {
          "salaryRange.to": {
        ...
        {
          "salaryRange.from": { $lte: parseInt(req.query.salaryFrom) },
          "salaryRange.to": { $gte: parseInt(req.query.salaryTo) },
        },
      ]
    });
  }
  ...
  const query = baseFilters.length > 1 ? { $and: baseFilters } : baseFilters[0];
  ...
  if (req.query.sortBy === "old") {
    sortStage = { $sort: { datePosted: 1 } }; // Oldest first
  } else {
    sortStage = { $sort: { datePosted: -1 } }; // Newest first (default)
  }

  // Use aggregation pipeline with sorting
    ...
    { $limit: limit },
    {
      $lookup: {
        ...
        pipeline: [
          {
            $project: {
              "userProfile.companyLogo": 1,
              "userProfile.companyName": 1
            }
          }
        ]
      }
  ...
  // Debug: Log first job to check if _id is present
  if (jobs.length > 0) {
    console.log("First job returned from DB:", JSON.stringify(jobs[0], null, 2));
  }
...
const getCompanies = asyncHandler(async (req, res) => {
  ...
  for (let company of companies) {
    if (company.userProfile.jobListings && company.userProfile.jobListings.length > 0) {
...
const getApplicationHistory = asyncHandler(async (req, res) => {
  ...
  if (role !== "jobSeeker") {
    throw new ApiError(403, "Only job seekers can view application history");
  }
  ...
  const appliedJobs = await Job.find({
    ...
    .select("title description location salaryRange type datePosted deadline isGeminiGenerated active")
    .sort({ datePosted: -1 });

  // Separate active and expired jobs
  const activeJobs = appliedJobs.filter(job => {
    if (job.isGeminiGenerated && job.deadline) {
      return new Date(job.deadline) > new Date();
    }
    return job.active;
  });

  const expiredJobs = appliedJobs.filter(job => {
    if (job.isGeminiGenerated && job.deadline) {
      return new Date(job.deadline) <= new Date();
    }
    return !job.active;
  ...
  return res.status(200).json(
    new ApiResponse(
      200,
      {
        ...
        expiredApplications: expiredJobs,
